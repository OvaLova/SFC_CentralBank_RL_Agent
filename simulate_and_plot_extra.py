...
plt.ylabel('Inflation Rate', fontsize=9)
...
plt.ylabel('Real GDP Growth', fontsize=9)
plt.legend()
plt.grid(True)
...
plt.ylabel('Interest Rate for Bills', fontsize=9)
plt.legend()
plt.grid(True)

# Plot real interest rates
plt.subplot(5, 1, 5)
r_b = np.array(history['r_b-1'])
pi = np.array(history['π_-1'])
rr_b = list((r_b - pi)/(1 + pi))
plt.plot(history['t'], rr_b, label='Real Policy Rate', color='green')
plt.ylabel('Real Interest Rate for Bills', fontsize=9)
plt.legend()
plt.grid(True)
...
plt.scatter(history['t'], history['action'], 
            ...
            zorder=3,
            s=15)     
plt.ylabel('Policy Action', fontsize=9)
